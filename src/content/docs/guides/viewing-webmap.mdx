...
todo:
  - Add downloadable AGS files zip
  - Repo for this demo? Folder inside bedrock-ge? Or downloadable zip
  - Add link to marimo notebook
  - Later: Marimo islands
---
...
<iframe src="/webmap/index.html" loading="lazy" width="100%" height="500" title="Bedrock AGS to GeoJSON Webmap demo" style="min-height:500px;" />

<LinkButton href="/webmap/index.html">View the webmap</LinkButton>

<LinkButton href="https://github.com/bedrock-engineer/ags-to-webmap/" variant='secondary'>View the code</LinkButton>

You can also explore the data preparation workflow interactively in a [marimo notebook](https://marimo.app/gh/bedrock-engineer/ags-to-webmap/main?entrypoint=data_processing%2Fags_to_geojson.py), a [reactive Python notebook](https://marimo.io/) that automatically updates when you modify data or code.
...
```python title="ags_to_geojson.py"
...
You can see the geospatial data in the `geodb.Location` table.
...
:::tip
To see what the headings in AGS mean, see the [Bedrock AGS4 reference](https://bedrock.engineer/reference/formats/ags/). Note there are slight differences between AGS3 and AGS4.
:::

You'll add some columns from the `geodb.Location` table.
Select the columns from the `geodb.Location` table:
...
Now merge these columns with the `geodb.LonLatHeight` table.
...
This joins the coordinate data with the borehole details. The `how="left"` ensures you keep all locations even if some don't have complete data. We drop the separate longitude/latitude columns since the geometry column already contains this information.

You can inspect your data at this stage:

```python title="ags_to_geojson.py"
print(locations_webmap_df.head())  # Show first few rows
print(f"Total locations: {len(locations_webmap_df)}")  # Count your boreholes
```

### 1.4: Export to GeoJSON

Now export the locations table to GeoJSON format for web display:
...
Create a new folder and move `locations.geojson` into it.

In this folder, create two new files: `index.html` and `script.js`
...
In the `index.html`, copy the following HTML.
...
<html lang="en">
  ...
  <body>
    <div id="map">
      <!-- This is the element that will become the map -->
    </div>
...
Now you'll build the interactive map. MapLibre GL JS is a web mapping library that displays maps and data in browsers.

#### Import the mapping library

In `script.js`, start with the import and configuration:

```js title="script.js"
import maplibregl from "https://cdn.jsdelivr.net/npm/maplibre-gl@5.6.2/+esm";

// Colors and names for your borehole types (matches AGS HOLE_TYPE values)
const agsHoleTypeConfig = {
  SCP: { color: "#e31a1c", name: "Standard Penetration Test" },
  "CP+RO+RC": { color: "#1f78b4", name: "CPT + Rotary Open + Rotary Cored" },
  "CP+RC+RO": { color: "#1f78b4", name: "CPT + Rotary Cored + Rotary Open" },
...
This configuration object maps your AGS hole types to colors and readable names for display. You can see what hole types are in your data with `geodb.Location["HOLE_TYPE"].unique()` and adjust the colors/names accordingly.

#### Create the map container

```js title="script.js"
// Create the map and tell it which HTML element to use
const map = new maplibregl.Map({
  container: "map",              // Uses the <div id="map"> from your HTML
  center: [114.2, 22.3],         // Starting view coordinates [longitude, latitude]
  zoom: 13,                      // Initial zoom level (higher = closer)
...
The `style` object defines what base map to show, here we're using [OpenStreetMap tiles](https://wiki.openstreetmap.org/wiki/Tiles) as the background.
...
map.on("load", () => {
  // Add the location GeoJSON file as a data source
  map.addSource("locations", {
    type: "geojson",
    data: "./locations.geojson",  // Points to the file you created
  });
...
Now add the visual styling. **This code continues inside the `map.on("load")` function:**

```js title="script.js"
  // Add a visual layer to display your boreholes as colored circles
  map.addLayer({
    id: "locations-layer",
    type: "circle",
    source: "locations",
    paint: {
      ...
      // Color each circle based on the HOLE_TYPE field
      "circle-color": [
        "match",
        ["get", "HOLE_TYPE"],              // Get HOLE_TYPE from each feature
        "SCP", agsHoleTypeConfig.SCP.color,
        "CP+RO+RC", agsHoleTypeConfig["CP+RO+RC"].color,
        "CP+RC+RO", agsHoleTypeConfig["CP+RC+RO"].color,
        "CP+RO", agsHoleTypeConfig["CP+RO"].color,
        "RO+CP", agsHoleTypeConfig["RO+CP"].color,
        "VC", agsHoleTypeConfig.VC.color,
        "#999999",                         // Default gray for unknown types
      ],
    },
  });
```
...
**Continue inside the `map.on("load")` function:**

```js title="script.js"
  // Show borehole details when clicked
  map.on("click", "locations-layer", (event) => {
    const { coordinates } = event.features[0].geometry;
    const [lon, lat] = coordinates;
    const properties = event.features[0].properties;
    const { HOLE_ID, HOLE_REM, HOLE_TYPE, HOLE_STAR, HOLE_ENDD } = properties;
...
<dl>
    <dt>Type</dt>
    <dd>${agsHoleTypeConfig[HOLE_TYPE]?.name || HOLE_TYPE}</dd>
  ...
  // Change cursor to pointer when hovering over boreholes
...
}); // Closes the `map.on("load")` function
```
...
Start your local server from the folder containing your files and visit `http://localhost:8000` to see your interactive map.

For permanent hosting, upload your files to any static hosting service.

## 3. Optional: Adding and displaying Geology & SPT data

Your basic map is already valuable for showing investigation coverage and key details.
For technical stakeholders who need to understand subsurface conditions, adding geology columns and SPT charts to the popups makes the map much more useful.

Let's add Standard Penetration Test & Geology data to the popup of each borehole location.
'Standard Penetration Test' data is in the `ISPT` group in AGS and 'Geology' is in the `GEOL` group in AGS.

You'll use the [Observable Plot](https://observablehq.com/plot/) library to draw a soil column and chart for SPT N values if present.

### 3.1: Grouping by Locations

You'll need to group the data by location.
Here's a convenience function that groups a table by location ID and writes it to a JSON file:
...
This groups all the in the given table records by borehole location. Each location gets its own list of test results. The `lambda` function converts each group into a list of dictionaries that JavaScript can easily read.
...
First, move the `geol.json` and `ispt.json` the folder of your web map.
...
Next, update your `script.js` to import the Observable Plot library and load the additional data files. Add this to the top of the file:
...
Finally, modify the click handler in your existing `script.js` to include the plotting code. Replace the existing popup creation section with this enhanced version:

```js title="script.js"
  // Show borehole details when clicked (replace the existing click handler)
  map.on("click", "locations-layer", (event) => {
    const coordinates = event.features[0].geometry.coordinates.slice();
    const properties = event.features[0].properties;
    const { HOLE_ID, HOLE_REM, HOLE_TYPE, HOLE_STAR, HOLE_ENDD, location_uid } = properties;

    // Get the geology and SPT data for this location
...
<dl>
    ...
    <dt>Coordinates</dt>
    <dd>${coordinates[1].toFixed(6)}, ${coordinates[0].toFixed(6)}</dd>
    ...
    // Add the soil column and SPT chart if data exists
    if (geology || spt) {
      const marks = [
        Plot.rect(geology, {
          y1: "depth_to_top",
      ...
      // Not every borehole has SPT data
      if (spt) {
        marks.push(
